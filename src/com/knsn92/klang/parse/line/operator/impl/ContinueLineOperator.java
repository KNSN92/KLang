...
public class ContinueLineOperator implements ILineOperator {

	@Override
	public boolean matches(Line line, Block block, int lineidx) {
		if(line.tokens.length != 1) return false;
		if(!TokenUtils.equals(line.tokens[0], KeywordToken.type, "continue")) return false;
		if(!isInLoopBlock(block)) {
			ErrorHandler.throwSyntaxError("The continue statement must always be written within a any loop statement.");
			return false;
		}
		return true;
	}
	...
	private static boolean isInLoopBlock(Block block) {
		if(ArrayUtils.contains(LineOperators.LOOPS, block.calledBy())) {
			return true;
		}
	...
	private static int getLoopBreakAmount(Block block) {
		if(ArrayUtils.contains(LineOperators.LOOPS, block.calledBy())) return 1;
